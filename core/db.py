...
def ensure_subject_allocation_schema(cur):
    # subject_topics (topics under a base subject, used by electives/CP)
    cur.execute("""
    ...
    """)
    cur.execute("CREATE INDEX IF NOT EXISTS idx_topics_subject ON subject_topics(subject_id)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_topics_code    ON subject_topics(topic_code)")

    # subject_alloc (one row per degree+batch+sem+branch+subject[+topic])
    cur.execute("""
        CREATE TABLE IF NOT EXISTS subject_alloc(
            ...
            subject_id      INTEGER NOT NULL,
            topic_id        INTEGER,
            sic_faculty_id  INTEGER,
            updated_at      TEXT    DEFAULT CURRENT_TIMESTAMP,
            /* Uniqueness over the logical identity (branch/topic can be NULL) */
            UNIQUE(degree_id, batch_year, semester,
                   IFNULL(branch_id,-1),
                   subject_id,
                   IFNULL(topic_id,-1))
        )
    """)
    cur.execute("CREATE INDEX IF NOT EXISTS idx_sa_degree_sem    ON subject_alloc(degree_id, semester)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_sa_degree_batch  ON subject_alloc(degree_id, batch_year)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_sa_subject_topic ON subject_alloc(subject_id, topic_id)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_sa_branch        ON subject_alloc(branch_id)")

    # subject_alloc_members (faculty per allocation row; role = lecture|studio)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS subject_alloc_members(
            id         INTEGER PRIMARY KEY AUTOINCREMENT,
            alloc_id   INTEGER NOT NULL,
            faculty_id INTEGER NOT NULL,
            role       TEXT    NOT NULL CHECK(role IN ('lecture','studio')),
            UNIQUE(alloc_id, faculty_id, role)
        )
    """)
    cur.execute("CREATE INDEX IF NOT EXISTS idx_sam_alloc ON subject_alloc_members(alloc_id)")
    cur.execute("CREATE INDEX IF NOT EXISTS idx_sam_fac   ON subject_alloc_members(faculty_id)")
