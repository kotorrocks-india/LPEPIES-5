    ...
    if ok:
        if not name.strip():
            ...
            try:
                ...
                # First, try the normal insert (works for modern schema with UNIQUE(degree_id, name))
                exec_sql(
                ...
                # Legacy DB: UNIQUE(name) hits or duplicate for this degree
                # Check if a row already exists with same name & same degree
                exists_same = read_df("SELECT id FROM branches WHERE name=? AND degree_id=?", (name.strip(), degree_id))
                if not exists_same.empty:
                    st.warning("Branch already exists for this degree.")
                else:
                    # If legacy UNIQUE(name) exists, disambiguate by appending degree name
    ...
    if dfb.empty:
        ...
        if st.button("Save Changes", disabled=not editable, key="br_save"):
            try:
                ...
                # If legacy UNIQUE(name) triggers across degrees, auto-disambiguate by suffixing degree
        ...
        if not cur.empty:
            ...
            if not nm.empty: current = str(nm.iloc[0]["name"])
    ...
    if st.button("Save Class In-Charge", disabled=not editable, key="save_cic"):
        try:
            with get_conn() as conn:
                ...
                # Build change set & check conflicts
                ...
                for y, nm in picks.items():
                    ...
                    if nm == "— None —":
                        ...
                        to_fid = int(core_fac[core_fac["name"] == nm]["id"].iloc[0])

                    # skip if no change
                    if (cur_fid or None) == to_fid:
                        continue

                    # global AY uniqueness for CIC (backstop: unique index too)
            ...
            if not need_cols.issubset(set(df.columns)):
                ...
                for _, r in df.iterrows():
                    ...
                    try:
                        ...
                        # try disambiguate legacy unique(name)
                        alt = f"{nm} ({pick_deg})"
                        try:
                            exec_sql("INSERT INTO branches(degree_id, name, start_year, end_year) VALUES(?,?,?,?)",
                                     (degree_id, alt, sy, ey))
